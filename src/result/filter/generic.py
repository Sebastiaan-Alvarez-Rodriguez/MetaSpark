...
def print_for_frame(frame):
    print('''
--- {}/{} cols used, {} nodes, {} partitions/node, extension '{}', compression {}
--- amount {}, kind {}, rb {} ({} measurements): {}
Total time: {:.3f}s  ({:.3f}s init, {:.3f}s compute)
Avg time:   {:.3f}s  ({:.3f}s init, {:.3f}s compute)
stddev:     {:.3f}s  ({:.3f}s init, {:.3f}s compute)
'''.format(
    frame.compute_cols, frame.num_cols, frame.node, frame.partitions_per_node, frame.extension, frame.compression,
    frame.amount, frame.kind, frame.rb, frame.size, frame.tag,
...
'''
def stats(resultdir, node, partitions_per_node, extension, compression, amount, kind, rb, skip_leading):
    path = fs.join(loc.get_metaspark_results_dir(), resultdir)

    reader = Reader(path)
    for frame_a, frame_b in reader.read_ops(node, partitions_per_node, extension, compression, amount, kind, rb, skip_leading):
