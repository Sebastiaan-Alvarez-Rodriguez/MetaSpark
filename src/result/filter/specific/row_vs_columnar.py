...
def stats(resultdir, num_cols, compute_cols, node, partitions_per_node, extension, compression, amount, kind, rb, large, no_show, store_fig, filetype, skip_leading):
    ...
    bplot0 = ax.bar([x-0.10 for x in indices], [np.median(x[1]) for x in plot_items], width, yerr=err0, capsize=6, label='Arrow-Spark', color=colors[0], alpha=0.75, edgecolor='black')
    bplot1 = ax.bar([x+0.10 for x in indices], [np.median(x[2]) for x in plot_items], width, yerr=err1, capsize=6, label='Spark', color=colors[1], alpha=0.75, edgecolor='black')
