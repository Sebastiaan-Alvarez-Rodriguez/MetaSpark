...
class BaseExperiment(ExperimentInterface):
    ...
    def init_params(self):
        ...
        --conf spark.eventLog.dir=/home/salvarez/spark-persist'.format(shared_submit_ops)
        ...
        self.runs = 4 # We run 1 times our implementation, and 1 times a Spark baseline implementation
        self.retries = 1 # If our application dies 3 times, we stop trying and move on
        self.appl_sleeptime = 120 # Sleep 120 seconds between checks
        self.appl_dead_after_tries = 2 # If results have not changed between 7 block checks, we think the application has died
    ...
    def deploy_data_fast(self, metadeploy, node, extension, amount, kind, rb, partitions_per_node, force_generate=False):
        command = 'rm -rf {} > /dev/null 2>&1'.format(loc.get_node_ram_dir())
    ...
    def do_experiment(self, metadeploy, node, extension, amount, kind, rb, partitions_per_node):
        outputloc = fs.join(self.resultloc(), node, partitions_per_node, extension, amount, kind, '{}.{}.{}.{}.{}.res'.format(node, extension, amount, kind, rb))
        ...
        for x in range(self.retries):
            ...
            if not metadeploy.deploy_application(self.jar, self.mainclass, runargs, self.extra_jars, self.submit_opts.format(outputloc), self.no_results_dir):
                printe('!! Fatal error when trying to deploy application !! ({})'.format(outputloc))
                return False

            if metadeploy.block(self.are_we_done_yet, args=(metadeploy, outputloc, runs_to_perform*2), sleeptime=self.appl_sleeptime, dead_after_retries=self.appl_dead_after_tries):
                return True # We are done!
            else: # Something bad happened. Do remaining runs in next iteration
                finished_runs = self.check_runs(outputloc)
                runs_to_perform -= ((finished_runs // 2)-1) #-1 because we need a 'cold buffer' run before going on
                outputloc += '_'+str(x)
            if x == self.retries-1:
                metadeploy.eprint('\n\n!!!FATALITY!!! for {}\n\n'.format(outputloc))
                print('\n\n!!!FATALITY!!! for {}\n\n'.format(outputloc))
